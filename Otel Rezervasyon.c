...
void rezervasyonTemizle(odaBilgileri *odalar,int odaNumarasi){
	...
	free(odalar[odaNumarasi-1].musteri.musteriAdi);//mustri adi ve soyadi için ayrýlan alanlarý bosalltýk
...
void doluOdaBilgileriYazdirma(odaBilgileri *odalar){
	...
	for(i=0;i<ODA_SAYISI;i++){
		if(odalar[i].odaDurumu!=0){//1 ise dolu 0 ise bos
		printf("---------------------------------------------------\n");
...
void odaBilgisiSorgulama(odaBilgileri *odalar,int odaNumarasi){
	if(odalar[odaNumarasi-1].odaDurumu!=0){//yani dolu
...
void fiyatBilgisiYazdirma(){
	...
	printf("Odemeniz gereken toplam tutar:%.2lfTL\n",toplamTutar);	
}
...
int main(){
	...
	odaDizisi=(odaBilgileri*)calloc(ODA_SAYISI,sizeof(odaBilgileri));//dinamik þekilde olusturduk
		...
		switch(ch){
			...
			case 'e':
				if(kullaniciYetkisi==personel){
					...
					printf("Bu alan sadece calisanlar tarafindan kullanilir.\n");
				}
				break;
			case 'S':
			case 's':
